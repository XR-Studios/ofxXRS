...
class ofxXRSTextInputField : public ofxXRSInteractiveObject{

    public:
    
        ofxXRSTextInputField()
        {
            mFocused = false;
            mTextChanged = false;
            mHighlightText = false;
            mMaxCharacters = 99;
            mType = ofxXRSInputType::ALPHA_NUMERIC;
        ...
        // center the text //
            ...
            // draw the input field background //
                if (mFocused && mType != ofxXRSInputType::COLORPICKER){
            ...
            // draw the text //
                ...
                ofSetColor(tColor);
                mFont->draw(mType == ofxXRSInputType::COLORPICKER ? "#" + mRendered : mRendered, tx, ty);
                if (mFocused) {
            // draw the cursor //
        ...
        void setTextInputFieldType(ofxXRSInputType type)
        {
            mType = type;
        }
        ...
        void onFocusLost()
        {
            ...
            if (mTextChanged){
                mTextChanged = false;
                ofxXRSInternalEvent e(ofxXRSEventType::INPUT_CHANGED, 0);
        ...
        void setCursorIndex(int index)
        {
           ...
           // if we're at a space append the width the font's '1' character //
            ...
            if (mType == ofxXRSInputType::COLORPICKER) mCursorX += mFont->rect("#").width;
            mCursorIndex = index;
        }
    ...
    protected:
        ...
        bool keyIsValid(int key)
        {
            if (key == OF_KEY_BACKSPACE || key == OF_KEY_LEFT || key == OF_KEY_RIGHT){
                return true;
            }   else if (mType == ofxXRSInputType::COLORPICKER){
            ...
            }   else if (mType == ofxXRSInputType::NUMERIC){
            // allow dash (-) or dot (.) //
                if (key==45 || key==46){
                    return true;
            // allow numbers 0-9 //
            ...
            }   else if (mType == ofxXRSInputType::ALPHA_NUMERIC){
